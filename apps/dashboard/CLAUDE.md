...
- Poll both endpoints on the same tick, on the interval in `.env.example` (`VITE_POLL_INTERVAL_MS`) — read it there rather than restating it here. That bar is the *sum* of four stages: proxy → IRIS, engine → proxy, the engine's own sustained-breach gap, then this poll. Each was chosen defensibly in isolation and nobody multiplied them out. **This stage is the one to spend**, and #44 is why: every upstream interval trades against something (the IRIS scrape rate, or the engine's `sustainedSeconds` debounce — which is MVP §6's false-positive protection and sets a hard floor at the engine's shipped interval, #64), while this one is gated by nothing. The engine serves from memory and no correctness property depends on how often we ask. Read the two upstream intervals from `services/metrics-proxy/.env.example` and `services/detection-engine/src/index.ts`, and the measured total from #44, rather than quoting any of them here — all three have moved at least once, and a duration copied into this file goes stale silently. #44 owns the bar itself.
...
- On error, back off by doubling the poll interval each time, capped at 30 s, and reset on the first success. Derived from the interval rather than a fixed sequence, so it moves when the interval does.
